...
class Spot:
    ...
    def is_closed(self):
        return self.color == RED #cannot be placed
    def is_open(self):
        return self.color == GREEN #whether current block which can be checked for placing queen
    def is_barrier(self):
        return self.color == BLACK #queen placed already
    def is_checking(self):
        return self.color == BLUE  #path visualisation
    def is_reset(self):
        return self.color == WHITE  #resetting the board
    ...
    def make_reset(self):
        self.color = WHITE
        #similar functions to set the colors of the blocks
    def draw(self,win):
        pygame.draw.rect(win, self.color, (self.x, self.y, self.width, self.width)) #creating a spot at given coordinates in window 
...
def isSafe(draw, grid, row, col):   #to check the corresponding row col and diagonal dont have any queen
    temp=grid[row][col].color
    grid[row][col].make_open()  #highlight the current block which needs to be checked
    draw()
    ...
    for i, j in zip(range(row, -1, -1), 
                    range(col, -1, -1)):    #zip function used for specifying range for nested loops
        if grid[i][j].is_barrier():     #if queen found then another queen cannot be placed hence return false
            grid[i][j].make_closed()     #since queen cannot be placed mark it as closed
            draw()
            grid[i][j].make_barrier()    #mark the spot back as barrier because spot is checked to barrier everytime
            draw()
            for i2, j2 in zip(range(row, -1, -1),   #grid cleanup
                    range(col, -1, -1)):
                    if i2==i and j2==j:
                        continue
            ...
            grid[row][col].make_reset()
            return False
        if not grid[i][j].is_open():        #highlighting the path
